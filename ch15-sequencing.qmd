...
**Foundation**: Based on **semi-conservative DNA replication** principle

**Key biological principle from Central Dogma**:
- DNA replication requires a template strand
- One strand serves as template (parent)
- New strand is synthesized complementary to template (daughter)
...
- **RNA polymerase** can start synthesis de novo (from scratch)
- **DNA polymerase** CANNOT start de novo
- **Why?** Evolutionary and structural reasons:

**Evolutionary explanation**:
1. **Error checking**: DNA polymerase has proofreading (3'→5' exonuclease activity)
   - Requires existing 3'-OH group to start
   - Can't proofread first nucleotide if starting de novo
   - RNA polymerase doesn't need same accuracy (RNA temporary)

2. **Substrate differences**:
   - RNA polymerase: Uses NTPs (with 2'-OH group)
   - DNA polymerase: Uses dNTPs (no 2'-OH group)
   - 2'-OH group helps in de novo initiation
   - DNA polymerase lost this capability

3. **Replication fidelity**:
   - Primer provides anchor point
   - Allows DNA polymerase to check base pairing
   - Reduces errors in replication

**This is why**:
- PCR needs primers!
- Sanger sequencing needs primers!
- DNA replication in cells uses RNA primers (from primase enzyme)

### The Chemistry of Sanger Sequencing

**Key components**:

**1. Normal dNTPs (deoxynucleotide triphosphates)**:
- dATP, dTTP, dGTP, dCTP
- Building blocks of DNA
- Have 3'-OH group (allows chain elongation)

**2. Chain-terminating ddNTPs (dideoxynucleotide triphosphates)**:
- ddATP, ddTTP, ddGTP, ddCTP
- **Missing 3'-OH group!**
- Can be incorporated but CANNOT extend further
- Labeled with fluorescent dyes (different color for each)

**Chemical structure difference**:
```
Normal dNTP:
  Base—Sugar(3'-OH, 5'-triphosphate)
           ↑
    Allows next nucleotide to attach

Terminating ddNTP:
  Base—Sugar(3'-H, 5'-triphosphate)
           ↑
    NO OH group → chain terminates!
```
...
**1. Primer annealing**:
```
Template: 5'---TACGGCATGCTA---3'
Primer:         3'-ATGC-5'
                    ↑
              DNA polymerase starts here
```

**2. Extension with mix of dNTPs and ddNTPs**:
- Mix contains:
  - Lots of normal dNTPs (dATP, dTTP, dGTP, dCTP)
  - Small amount of labeled ddNTPs (ddATP-fluorescent, etc.)
- DNA polymerase incorporates nucleotides randomly
- Sometimes normal dNTP → chain continues
- Sometimes ddNTP → chain terminates!

**3. Products formed** (example):
```
Template: 5'-TACGGCATGCTA-3'

Terminated fragments:
3'-ATGCCGddT-5'        (terminated at position 7 with ddT)
3'-ATGCCGTAddC-5'      (terminated at position 8 with ddC)
3'-ATGCCGTACddG-5'     (terminated at position 9 with ddG)
3'-ATGCCGTACGddA-5'    (terminated at position 10 with ddA)
3'-ATGCCGTACGA-ddT-5'  (terminated at position 11 with ddT)

Each fragment labeled with color corresponding to last nucleotide!
```

**4. Separation by size**:
- **Capillary electrophoresis** (modern method)
- DNA fragments migrate through thin capillary
- Smaller fragments move faster
- Separated by single nucleotide difference!
- Laser detects fluorescent ddNTP at end of each fragment

**5. Detection**:
```
Time → Fragment size increases

Detector sees:
Color Blue (ddT, position 7)
Color Red (ddC, position 8)
Color Green (ddG, position 9)
Color Yellow (ddA, position 10)
Color Blue (ddT, position 11)
